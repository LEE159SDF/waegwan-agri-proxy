...
export default async function handler(req, res) {
  try {
    ...
    const KEY_ENC = process.env.DATA_GO_KEY_ENC || ''; // 선택: Encoding 키(%2F...%3D%3D)
    ...
    qs.set('serviceKey', KEY_ENC || encodeURIComponent(KEY_DEC)); // 이중 인코딩 방지
    qs.set('Page_No', String(pageNo));
    qs.set('Page_Size', String(pageSize));
    qs.set('date_Time', String(date));
